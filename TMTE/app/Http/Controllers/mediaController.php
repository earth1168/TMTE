...
class mediaController extends Controller
{
    ...
    {    //everytime add/change mediaLog

        $temp = explode(',', $request->pID);
        $mediaID = $temp[0];
        $pID = $temp[1];
        $mediaLogID = DB::select(
            DB::raw('SELECT id FROM media_logs WHERE media_logs.mediaID = :mediaID AND media_logs.profileID = :pID'),
            array('mediaID' => $mediaID, 'pID' => $pID)
        );

        $mediaData = DB::select(
            DB::raw('SELECT * FROM media WHERE media.id = :mediaID'),
    ...
    public function searchMedia(Request $request)
    {
        ...
        if ($kidOrNot[0]->kidUser == 1) {
            if ($mFilter == 'Everything') {
                $data = DB::table('media')->where([['mediaName', 'like', '%' . $mText . '%'],
                                                   ['kid', 1]])->get();
